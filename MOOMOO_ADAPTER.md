...
| `src/moomoo_login.py` | New. Browser login (OAuth 2.1 with PKCE, `quote:read` only) that stores a token for the engine; the same consent page the MCP clients use. |
...
the adjusted series it computes is the vendor's forward-adjusted series unchanged. Forward
adjustment on moomoo folds in cash dividends and splits, which is the same convention as
Yahoo's `Adj Close`. Snapshot CSVs from this source therefore hold forward-adjusted prices,
not raw prints; the manifest's `price_source` says which source produced each snapshot.
...
2. **The engine.** The Python run needs its own token. Log in once in the browser:
   ...
   It binds a local callback port first, then opens the same moomoo consent page requesting
   only the `quote:read` scope. A grant with any broader scope is refused rather than stored,
   and the engine refuses to use a stored token whose scope is broader. The token lives under
   `.cache/moomoo/` (ignored by git, owner-only from the moment it is created) and refreshes
   itself; if a refresh ever fails, the run stops calling moomoo and tells you to log in again.
   If the default port is busy, pass `--port 0`. No password is typed into the terminal, ever.
...
The replay used the same forward-adjusted bars the adapter now requests. A full 51-name live
