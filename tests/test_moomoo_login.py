...
class LoginTests(unittest.TestCase):
    ...
    def test_quote_read_is_required_and_additional_scopes_are_accepted(self):
        scopes = "quote:read accid:283445331361909406 quote:write trade:read trade:write"
        self.assertEqual(moomoo_login._token_response(FakeResponse(200, {"access_token": "a", "scope": scopes}))["access_token"], "a")
        moomoo_login.save_token(good_store(scope=scopes), self.path)
        self.assertEqual(moomoo_login.load_token(self.path)["scope"], scopes)
        for scope in (None, "", "quote:write", "trade:read accid:283445331361909406"):
            with self.assertRaisesRegex(RuntimeError, "missing required quote:read"):
                moomoo_login.check_scope(scope)
    ...
    def test_full_login_flow_binds_before_browser_registers_exchanges_and_stores(self):
        ...
        self.assertNotIn("scope", captured["q"]); self.assertEqual(captured["q"]["code_challenge_method"], "S256")
    ...
    def test_connect_prefers_environment_then_stored_login_then_explains(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict(data_moomoo.os.environ, {}, clear=True), patch.object(moomoo_login, "TOKEN_FILE", path):
                ...
                moomoo_login.save_token(good_store(access_token="broader", scope="quote:read trade:read accid:283445331361909406"), path)
                session = data_moomoo.connect()
                self.assertEqual(session.access_token, "broader")
