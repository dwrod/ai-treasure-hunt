...
1. **The engine's data source.** If `--source moomoo` fails with
   ...
   and tell the user to choose a grant that includes quote read in the browser
   that opens. Moomoo's consent page determines the grant; extra scopes and
   account context are accepted, but the engine itself calls quote endpoints
   only. The token is stored under .cache/ and refreshes itself afterwards. If
   the port is busy, add `--port 0`. Environment variables `MOOMOO_ACCESS_TOKEN`, or
