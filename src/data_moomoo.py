...
One request per symbol: the production endpoint's forward-adjusted daily bars.
Its currently accepted forward mode adjusts for splits but excludes cash dividends.
Adj Close is set equal to Close, so the engine's adjustment factor is one and the
adjusted series it computes is the vendor's returned series unchanged. Snapshot
CSVs written from this source therefore hold forward-adjusted prices, not raw prints.
...
KLINE_DAY = 2
ADJUST_FORWARD = 1  # Production API forward adjustment (splits; excludes cash dividends).
