...
The flow is OAuth 2.1 with PKCE against the documented endpoints. Only the
`quote:read` scope is requested, and a token granted with any broader scope is
refused rather than stored. No password is ever typed here; you log in on
...
SCOPE = "quote:read"                 # the only scope this tool requests or accepts
...
def check_scope(scope):
    """Refuse any grant beyond quote:read; a broader token must never be stored or used."""
    granted = set((scope or SCOPE).split())
    extra = granted - ALLOWED_SCOPES
    if extra:
        raise RuntimeError(f"moomoo granted scopes this tool does not accept ({' '.join(sorted(extra))}); token not stored")
...
def login(port=DEFAULT_PORT, timeout=300, http=None, open_browser=webbrowser.open, path=None):
    ...
    store = merge_token({"client_id": client_id, "redirect_uri": redirect_uri, "token_endpoint": urls["token_endpoint"],
                         "scope": SCOPE}, data)
    save_token(store, path)
    return store
...
def main(argv=None):
    parser = argparse.ArgumentParser(description=f"Log in to moomoo in your browser and store a {SCOPE} token for --source moomoo.")
    ...
    print(f"Logged in. Scope: {store.get('scope', SCOPE)}. Token stored at {TOKEN_FILE.relative_to(ROOT)}.")
